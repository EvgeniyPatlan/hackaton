...
export class AnalyticsRepository {
  ...
  async upsertLocationAnalytics(
    ...
    // Спочатку пошук запису за locationId
    const existingRecord = await this.prisma.locationAnalytics.findFirst({
      where: { locationId: locationId },
      select: { id: true }
    });

    if (existingRecord) {
      // Оновлення існуючого запису за id
      return this.prisma.locationAnalytics.update({
        where: { id: existingRecord.id },
        data,
      });
    } else {
      // Створення нового запису
      return this.prisma.locationAnalytics.create({
        data: {
  ...
  async getLocationAnalyticsById(locationId: string) {
    return this.prisma.locationAnalytics.findFirst({
      where: { locationId: locationId },
    });
  }
